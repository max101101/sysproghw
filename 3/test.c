...
test_io(void)
{
	...
	unit_check(ufs_write(fd, data, size) == size, "data is written");

	char buffer[2048];
	unit_check(ufs_read(fd, buffer, sizeof(buffer)) == size,
		   "data is read");
...
test_delete(void)
{
	...
	unit_check(ufs_write(fd3, "bc", 2) == 2, "into another the same fd");
	...
	unit_check(fd4 == -1, "despite the existing 'ghost' is not visible "\
		   "anymore for new descriptors");
	unit_check(ufs_errno() == UFS_ERR_NO_FILE, "errno is set");
